...
export abstract class ShapeBuilder<T extends Shape> {
  ...
  abstract onUpdated: (() => void) | undefined;
  ...
  setOptions(element: ElementWithExtra, categories: string[], color?: string) {
    ...
    if (labeled) {
      ...
      element.categoriesRect = this.svg
        .rect(width, height)
        .radius(2)
        .move(pos[0] - width / 2, pos[1] + height / 4)
        .fill(color ? `${color}30` : "#ffffff80");
      element.categoriesPlain.remove();
      element.categoriesPlain = this.svg
        .plain(categoriesPlain)
        .move(pos[0], pos[1])
        .font({fill: "#000000", size: 12, anchor: "middle", weight: "bold", family: "Arial"})
  ...
  drag_mm(e: MouseEvent) {
    if (this.lastPoint) {
      ...
      let dx = e.offsetX - this.lastPoint.X,
