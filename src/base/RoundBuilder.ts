...
export abstract class RoundBuilder<
  T extends RoundShape
> extends ShapeBuilder<T> {
  element?: IlEllipse;
  origin?: Point;
  points: ArrayXY[] = [];
  canHB = true;
  ...
  createElement(shape: RoundShape): void {
    ...
    this.element
      ...
      .move(
        shape.centre[0] - shape.width / 2,
        shape.centre[1] - shape.height / 2
      );
  ...
  draw_md(e: MouseEvent, addShape: () => void) {
    if (e.buttons === 1 && !e.ctrlKey && !this.origin) {
      ...
      this.svg
        .mousemove((e: MouseEvent) => this.draw_mm(e, addShape))
  ...
  draw_mm(e: MouseEvent, addShape: () => void): void {
    if (this.origin) {
      ...
      let centre: ArrayXY = [
        (this.origin.X + e.offsetX) / 2,
        (this.origin.Y + e.offsetY) / 2
      ];
      let radius = this.calculateRadius([e.offsetX, e.offsetY]);
      [this.element!, this.element!.shadow].forEach((el) =>
        el
          .size(2 * radius[0], 2 * radius[1])
          .move(centre[0] - radius[0], centre[1] - radius[1])
      );
    }
  }
  ...
  plot(ellipse: IlEllipse): void {
    [ellipse, ellipse.shadow].forEach((el) =>
      el
        .size(ellipse.shape.width, ellipse.shape.height)
        .move(
          ellipse.shape.centre[0] - ellipse.shape.width / 2,
          ellipse.shape.centre[1] - ellipse.shape.height / 2
        )
    );
  }
      ...
      _disc
        ...
        .mousedown((e: MouseEvent) => {
          if (e.buttons === 1 && !e.ctrlKey && this.dragIndex === undefined) {
            ...
            [this.movePath!, ...this.rotateArr].forEach((item) =>
              item.remove()
            );
            e.stopPropagation();
  ...
  addDiscs(): void {
    ...
    this.setPoints();
    elem.discs = this.points.map((point) =>
      this.drawDisc(point[0], point[1], this.sd.discRadius, Color.GreenDisc)
    );
    elem.connector = this.svg
      .polyline([...this.points, this.points[0]])
      .fill(Color.ShapeFill)
      .stroke({
        color: Color.BlackLine,
        width: 1,
  ...
  setPoints() {
    let elem = this.element!,
      width = elem.shape.width,
      height = elem.shape.height,
      x = elem.shape.centre[0],
      y = elem.shape.centre[1];
    this.points = [
  ...
  editShape_mm(e: MouseEvent) {
    if (this.dragIndex !== undefined) {
      ...
      let prevIndex = this.dragIndex === 0 ? 3 : this.dragIndex - 1,
        ...
        oppositIndex =
          this.dragIndex > 1 ? this.dragIndex - 2 : this.dragIndex + 2;
      this.origin = {
        X: this.points[oppositIndex][0],
        Y: this.points[oppositIndex][1]
      };
      let diff = this.calculateDifferent([x, y]);
      this.points[this.dragIndex] = [
        diff[0] + this.origin.X,
        diff[1] + this.origin.Y
      ];
      ...
      elem.shape.centerChanged(
        Util.rotate(
          [this.origin.X + diff[0] / 2, this.origin.Y + diff[1] / 2],
          oldCenter,
          phi
        )
      );
      ...
      elem.discs.forEach((disc, i) =>
        disc.move(
          this.points[i][0] - discRadius,
          this.points[i][1] - discRadius
        )
      );
